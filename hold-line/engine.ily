  ...
  (let* ((hl-grob   (ly:engraver-make-grob translator 'HoldLine note-ev)))
   ...
   (if (and (not (remove? hold-line))
            (not (< (ly:grob-staff-position parent-grob)
                    (getOption '(lute-tab hold-line diapasonLevel)))))
    (begin
...
#(define (end-hl-grob x hold-line translator duration)
  (let ((grob         (car x))
   ...
   (if (getOption '(lute-tab hold-line attachToClosest))
    (let* ((closest-grob  '())
   ...
   (if (getOption '(lute-tab hold-line flatLines))
    (flatten-line grob)
...
#(define (maybe-adjust-bound grob right-bound hold-line)
  ...
  (let*  ((left-bound        (ly:spanner-bound grob LEFT))
   ...
   (if (< right-staff-space (getOption '(lute-tab hold-line diapasonLevel)))
     (flatten-line grob)
     (ly:grob-set-nested-property! grob '(bound-details right attach-dir) LEFT))
))
