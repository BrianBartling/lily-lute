  ...
  (let* ((hl-grob   (ly:engraver-make-grob translator 'HoldLine note-ev)))
   ...
   (if (and (not (remove? hold-line))
            (not (< (ly:grob-staff-position parent-grob)
                    (getOption '(tab-tools hold-line diapasonLevel)))))
    (begin
...
#(define (maybe-adjust-bound grob right-bound hold-line)
  ...
  (let*  ((left-bound        (ly:spanner-bound grob LEFT))
   ...
   (format #t "right-staff-space: ~a, left-staff-space: ~a\n" right-staff-space left-staff-space)
   (if (and (not (null? (ta-event (prev-hold-line hold-line))))
            (!= distance 0))
    (begin
     (format #t "right here\n")
     (format #t "right offset: ~a\n" right-offset)
     (format #t "left offset: ~a\n" (ly:grob-property left-bound 'Y-offset))
     (format #t "right extent: ~a\n" right-extent)
   ...
   (if (and (> right-staff-space left-staff-space)
    ...
    (let  ((fng-extent     (* (abs 
     ...
     (format #t "right-extent: ~a\n" right-extent)
     (format #t "right-offset: ~a\n" right-offset)
     (format #t "Y: ~a\n" (grob-nested-property grob '(bound-details right Y)))
     (format #t "current offset: ~a\n" current-offset)
     (format #t "fng-extent: ~a\n" fng-extent)
